...
class TestBenchmarkLongHorizon:
    ...
    def test_build_neural_models_uses_default_models(self) -> None:
        ...
        models = _build_neural_models(horizon=2, max_steps=7, freq="D")
    ...
    def test_run_command_uses_mode_defaults_by_default(
        self,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """The CLI should use the built-in default benchmark settings."""
        runner = CliRunner()
        benchmark_mock = Mock(
        ...
        assert benchmark_mock.call_args.kwargs["max_steps"] == 50
    ...
    def test_run_command_rejects_removed_auto_options(self) -> None:
        """The CLI should not expose auto-search tuning options anymore."""
        runner = CliRunner()

        result = runner.invoke(
            benchmark_long_horizon.app, ["run", "--auto-preset", "smoke"]
        )
